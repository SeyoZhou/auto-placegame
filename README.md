...
`world-boss` 是独立命令，只在北京时间 `10:00–11:00`、`16:00–17:00`、`20:00–21:00` 内工作，绝不会由 `run` 或 `daily` 间接触发。它启动时检测主机 IANA 时区和 UTC 偏移，再换算到 `Asia/Shanghai`；每场按低等级 Boss 优先，最多并发处理 3 个账号，并让每个账号对每个可协作的世界首领各提交 1 次协助。每个账号与首领组合的提交标记会在请求前持久化，避免响应不明确或进程重启后重复协助。只有服务端明确返回 `rewardStatus: "claimable"` 时才领奖。
