...
export function chooseBestEquipmentUpgrade(equipment = [], excludedSlots = new Set()) {
  const slots = equipmentBySlot(equipment);
  const upgrades = [];
  for (const [slotName, items] of slots) {
    if (excludedSlots.has(slotName)) continue;
    if (items.candidates.length === 0 || items.equipped.length > 1) continue;
    ...
    const candidate = items.candidates[0];
...
export function equipmentComparisonIssue(equipment = []) {
  for (const [slot, items] of equipmentBySlot(equipment)) {
    ...
    if (items.bagItems.some((item) => equipmentScore(item) === undefined)) {
      return { slot, reason: "candidate-score-unknown" };
    }
  }
  return undefined;
}
...
export function personalBossPreviewLayers(bosses = [], equipment = []) {
  ...
  for (const boss of personal) {
    ...
    const skillKeys = arrayKeys(options.skills);
...
export function chooseBossPreview(previews = [], minimumChance = 80) {
...
export function safeDecompositionCandidates(equipment = [], settings = {}) {
  const qualities = new Set(Array.isArray(settings.qualities) ? settings.qualities : []);
  const minLevel = optionalFinite(settings.minLevel, -Infinity);
  const maxLevel = optionalFinite(settings.maxLevel, Infinity);
  const candidates = equipment.filter((item) => {
    return decompositionBaseEligible(item, qualities, minLevel, maxLevel)
...
function decompositionBaseEligible(item, qualities, minLevel, maxLevel) {
  const level = Number(item?.level);
  const score = Number(item?.score ?? item?.power);
