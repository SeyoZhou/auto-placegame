...
const ASSIST_LIMIT_PER_PAIR = 1;
const ASSIST_POLICY = "one-assist-per-boss-v1";

export function worldBossEventIsComplete(eventState) {
  return eventState?.status === "completed" && eventState.assistPolicy === ASSIST_POLICY;
}
...
export async function executeWorldBossSession({
  ...
  if (typeof allowEventCompletion !== "boolean") throw new Error("world boss completion permission must be boolean");
  ...
  if (!dryRun) {
    ...
    eventState.assistPolicy = ASSIST_POLICY;
  ...
  const completed = allowEventCompletion && priorities.length > 0 && !unresolved;
  if (!dryRun) {
...
async function drainBossPair({ client, initial, eventState, report, persist, dryRun }) {
  ...
  const legacySubmissionForPair = account.assistSubmitted === true && account.assistBossKey === initial.bossKey;
  if (recorded?.submissionStarted === true || legacySubmissionForPair) {
    report.actions.push(worldBossAction("world-boss-assist", recorded?.status === "failed" ? "failed" : "uncertain", {
      bossKey: initial.bossKey,
      instanceId: initial.instanceId,
      reason: "recorded-submission"
    }));
    return;
  }
  if (dryRun) {
    report.actions.push(worldBossAction("world-boss-assist", "planned", {
      bossKey: initial.bossKey,
      instanceId: initial.instanceId,
      attempts: ASSIST_LIMIT_PER_PAIR
    }));
    return;
  }
  ...
  const remainingUnchanged = Number.isInteger(after?.remainingAttemptCount)
    && after.remainingAttemptCount === before.remainingAttemptCount;

  if (mutationError && !(mutationError.ambiguous && remainingDecreased)) {
    if (!mutationError.ambiguous && remainingUnchanged) delete pair.submissionStarted;
  ...
  await completePair(pair, report, persist, before, "one-attempt-limit", false);
}
