...
const ASSIST_LIMIT_PER_ACCOUNT = 1;
...
export async function executeWorldBossSession({
  ...
  const dryRunPlannedAccounts = new Set();
...
async function drainBossPair({ client, initial, eventState, report, persist, dryRun, dryRunPlannedAccounts }) {
  ...
  const legacySubmission = Object.values(account.pairs).some((pair) => {
    return Number(pair?.attempts) > 0 || pair?.status === "uncertain";
  });
  if (account.assistSubmitted === true || legacySubmission) {
    report.actions.push(worldBossAction("world-boss-assist", "skipped", {
      bossKey: initial.bossKey,
      instanceId: initial.instanceId,
      reason: "account-attempt-limit"
    }));
    return;
  }
  if (dryRun) {
    if (dryRunPlannedAccounts.has(client.alias)) {
      report.actions.push(worldBossAction("world-boss-assist", "skipped", {
        bossKey: initial.bossKey,
        instanceId: initial.instanceId,
        reason: "account-attempt-limit"
    ...
    dryRunPlannedAccounts.add(client.alias);
    report.actions.push(worldBossAction("world-boss-assist", "planned", {
      bossKey: initial.bossKey,
      instanceId: initial.instanceId,
      attempts: ASSIST_LIMIT_PER_ACCOUNT
    }));
    return;
  }
  ...
  let mutationError;
  account.assistSubmitted = true;
  account.assistBossKey = before.bossKey;
  ...
  await completePair(pair, report, persist, before, "account-attempt-limit", false);
}
